...
def get_current_user(authorization: Optional[str] = Header(None)) -> dict:
    """
    Parse Bearer token va tra ve user info.
    Raise HTTPException 401 neu token khong hop le.
    """
...
def require_permission(resource: str, action: str):
    """
    Decorator kiem tra RBAC permission.
    Dung casbin enforcer de check (role, resource, action).
    Raise HTTPException 403 neu khong co quyen.
    """
    def decorator(func):
        @wraps(func)
